...
2. **Analysis (tagging and bucketing).** Claude reasoning plus Claude vision (vision reads competitor static images directly), routed through `../ad-analysis-tagger/SKILL.md` for the six-dimension breakdown (its Dimension 2B is the single authority for visual-style assignment, drawing on the `../ad-style-generator/references/style-catalogue.md` catalogue; this skill reads that assignment and does not re-assign), and the `../video-script-generator/SKILL.md` format set for video format matching.
...
**Principle.** Generation transposes the concept, it does not duplicate the ad. Keep what makes the original work (its structure and its single `distinctive_device`, the tagger's named output). Strip the source brand's identity entirely (its palette, fonts, product form, and naming). Rebuild the concept in the target brand's own identity, using the target brand's real product. A faithful copy of a competitor ad is a failure, not a success.
...
4. **Scan each survivor.** For each ad in the top N, call `scan_ad` with the Meta Ad Library URL or the ad id. Capture the image asset URL on `medias.trendtrack.io`, the reach, the days running, the scaling verdict, and the ad primary text (`content.body` from the ads response; the ads response carries the ad primary text on statics and videos, and is null only on genuinely textless ads). Always record the primary text for every ad, using null when `content.body` is absent, so Step B.5 can either forward it to the tagger or declare its absence. [v0.14 sd-wave]
5. **Tag and bucket -- PAUSE gate (Desktop).** `scan_ad` returns an image asset URL, not a viewable image, and vision cannot read a bare URL.
   ...
   **STOP HERE. Do not proceed to tagging until images are in hand.**
   ...
   Once the operator returns all images, tag each ad via `../ad-analysis-tagger/SKILL.md` across its six dimensions. Pass the tagger the exact analysis-purpose enum value `transposition extraction` (the tagger's transposition-extraction purpose), which activates Dimension 5B emphasis and populates the named `distinctive_device` output that the TRANSPOSE path consumes. [v0.14 sd-wave]

   Pass the returned image as the visual input together with the Step B.4 ad primary text (input type 'Long-form static (image + Facebook in-feed primary text)'). For every ad, check that Step B.4 primary text: when it is present, forward it to the tagger with the image; when it is null (`content.body` absent on a genuinely textless ad), state the degradation explicitly in the tagged breakdown with a declared 'primary text absent from source' line for that ad, and do not let the tagger's silent degraded ('Static image only') mode stand alone. This check runs on every ad, so a null-body ad always carries the declared line. [v0.14 sd-wave]

   Visual-style assignment is the tagger's Dimension 2B, which is the single authority: read the style the tagger assigns and its `Custom-visual` determination, and do not re-assign the style here. Add only the across-the-set bucketing view: group the ads by their tagger-assigned styles, and surface any `Custom-visual` (candidate-new-style) the tagger flagged, with a one-line rationale. Do not raise an independent candidate-new-style flag. [v0.14 sd-wave]
   ...
   - Score 1-2 (TRANSPOSE path): the layout is not the replication target. Extract the concept and the tagger's `distinctive_device`, rebuild creatively in the target brand's identity. The classification is not produced at this score.
...
7. **Generate (on request): the concept-transposition pipeline.** Generation is opt-in per run, not automatic. Follow the Generation method section above (transpose, do not duplicate).
   ...
   **The moves (prompt construction and generation; blocked until the gate report passes and the move-6 scan passes):**
   1. Confirm `effective_execution_path` (NOT the raw Step B.5 path; on a downgrade this is TRANSPOSE). Prompt construction reads layout ONLY from `effective_layout_lock` and `prompt_required_literals`; the camera angle comes SOLELY from `resolved_camera_angle` (Gate 3's emitted field), never from the tagger's raw clone_aspects and never from `effective_layout_lock.camera_angle` (which is internal pre-resolution state after Gate 3). Apply the per-path handling:
      ...
      - TRANSPOSE: extract the transposable concept and the tagger's `distinctive_device` (the element that makes the ad work), and build to the concept-only `effective_layout_lock`.
   ...
   3. Device-analog preservation. If the `distinctive_device` depends on the source product's physical form, find the target product's honest analog rather than copying the form or dropping the device. Worked example: a competitor supplement ad whose device was cracking a capsule to expose the powder transposes to a shampoo brand as the formula pouring and swirling from the intact bottle.
...
4. **Build the transcript brief, then hand off to operator for Gemini visual pass -- PAUSE gate.**
   ...
   **Level 2 -- Content validation (field completeness, per section):**
   ...
   If both levels pass for all videos: proceed to tagging. Tag the combined brief (transcript brief + Gemini visual analysis) for each video via `../ad-analysis-tagger/SKILL.md` with the same exact analysis-purpose enum value `transposition extraction` used in the statics lane (so each video analysis produces the named `distinctive_device` that the script step in Step C.6 consumes; require it before Step C.5 fit assessment or Step C.6 script generation), and as the 'Transcript + visual brief (textual)' input type, per the tagger's Input-Type Output Sub-Format table: visual-style tagging is permitted from the described visuals but flagged text-derived, and Dimension 5B is not produced (not-produced reason 'non-static input'). [v0.14 sd-wave] Then match-and-flag against the `../video-script-generator/SKILL.md` format set (fits an existing format, or candidate new format with one-line rationale). The Gemini brief is source-side analysis only; the applied layer is produced by the generation pipeline after the target brand kit is loaded, not by this pass.
