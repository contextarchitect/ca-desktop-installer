...
| **Default aspect ratio** | Landscape (1200x800) or hero (1920x1080); infographics may be portrait | 1:1 (1080x1080) per ad-style-generator default |
...
3. **Image dimensions match funnel format, not ad format.** Funnel inline images use landscape ratios (1200x800 typical) or hero ratios (1920x1080); funnel infographics may be portrait. Ad images default to 1:1 (1080x1080) per ad-style-generator's aspect ratio rule. If the images you have in context are 1:1, they are ad images, not funnel images.
...
- **Image URLs and download instructions (MANDATORY):**
  - Include the public URL of each funnel image from Stage 2 (typically Kie.ai temporary URLs)
  - **CRITICAL: Instruct Lovable to download each image locally to the project assets.** Kie.ai URLs are temporary (typically 24-48 hours). If Lovable references them directly via `<img src="[URL]">` without downloading, the images will 404 once the URLs expire.
  - Sample wording to include in the Lovable prompt: "Download each of the following images and save them to the project's image assets folder. Reference them locally in the rendered page. Do NOT hotlink the URLs because they are temporary and will expire."
